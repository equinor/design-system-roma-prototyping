...
import page from './ChipRl.docs.mdx'
import { ChipRl } from './ChipRl'
...
import type { ChipRlProps, ChipRlColor } from './ChipRl.types'
import type { BadgeColor } from '../Badge/Badge.types'

type StoryArgs = ChipRlProps & { color?: ChipRlColor }

const meta: Meta<StoryArgs> = {
  title: 'EDS 2.0 (beta)/Data Display/ChipRl',
  component: ChipRl,
  parameters: {
    docs: {
      page,
...
<ChipRl ref={chipRef} color={selected.chipColor} dropdown onClick={() => setMenuOpen(o => !o)}>
  {selected.label}
</ChipRl>
...
export const Assist: Story = {
  render: () => (
    <Card elevation="raised" style={{ maxWidth: 340 }}>
      ...
      <Card.Content>
        <Wrapper>
          <ChipRl onClick={() => {}} icon={calendar_event}>
            Add to calendar
          </ChipRl>
          <ChipRl onClick={() => {}} icon={directions}>
            Get directions
          </ChipRl>
          <ChipRl onClick={() => {}} icon={share}>
            Share event
          </ChipRl>
        </Wrapper>
      </Card.Content>
...
<ChipRl onClick={handleAction} icon={calendar_event}>Add to calendar</ChipRl>
<ChipRl onClick={handleAction} icon={directions}>Get directions</ChipRl>
<ChipRl onClick={handleAction} icon={share}>Share event</ChipRl>`,
      },
    },
  },
}
...
<ChipRl ref={setAnchorEl} onClick={() => setOpen(!open)} dropdown selected={hasSelection}>
  {chipLabel}
</ChipRl>
...
{/* Boolean filter chip */}
<ChipRl onClick={() => setCompleted(!completed)} selected={completed}>
  Is completed
</ChipRl>`,
      },
    },
...
const ActiveFilters = () => {
  ...
  return (
    <Wrapper>
      ...
      {filters.map((filter) => (
        <ChipRl key={filter.id} onDelete={() => remove(filter.id)}>
          {filter.label}
        </ChipRl>
      ))}
    </Wrapper>
  ...
  return (
    <Wrapper>
      {people
        ...
        .map((person) => (
          <ChipRl key={person.id} onDelete={() => remove(person.id)}>
...
<ChipRl onDelete={() => remove(person.id)}>
  <Avatar src={person.img} alt={person.name} size={16} />
  {person.name}
</ChipRl>`,
      },
    ...
    >
      ...
      >
        {suggestions.length > 0 && (
          ...
          >
            {suggestions.map((s) => (
              <ChipRl key={s} onClick={() => handleSuggestion(s)}>
                {s}
              </ChipRl>
            ))}
          </div>
...
{replies.map(reply => (
  <ChipRl key={reply} onClick={() => sendReply(reply)}>
    {reply}
  </ChipRl>
))}`,
      },
...
const ShippingTags = () => {
  ...
  return (
    <Wrapper>
      {tags.map((tag) => (
        <ChipRl key={tag.label} color={tag.color} onDelete={() => remove(tag.label)}>
          {tag.label}
        </ChipRl>
      ))}
    </Wrapper>
  ...
  render: () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div>
        ...
        <Wrapper>
          <ChipRl onClick={() => {}}>Default</ChipRl>
          <ChipRl onClick={() => {}} selected>
            Selected
          </ChipRl>
          <ChipRl onClick={() => {}} icon={save}>
            With icon
          </ChipRl>
          <ChipRl onDelete={() => {}}>Deletable</ChipRl>
        </Wrapper>
      </div>
...
<ChipRl onClick={handleClick}>Default</ChipRl>
